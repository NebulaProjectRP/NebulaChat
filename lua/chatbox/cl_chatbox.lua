...
cvars.AddChangeCallback("lounge_chat_w", function(cvar, old, new)
	if (IsValid(_LOUNGE_CHAT)) then
		...
		_LOUNGE_CHAT.m_Close:AlignRight(8)
...
function LOUNGE_CHAT:CreateChatbox()
	...
	_LOUNGE_CHAT = frame
		...
		local close = vgui.Create("nebula.close", frame)
		...
		options.Paint = function(me, w, h)
			...
			surface.SetDrawColor(c.r, c.g, c.b, frame:GetAlpha() * (me.Hovered and 1 or 0.5))
		...
		local history = vgui.Create("nebula.scroll", frame)
		...
		frame.m_Bottom = bottom
			...
			local entry = vgui.Create("nebula.textentry", bottom)
			...
			emoticons.opaint = emoticons.Paint
...
function LOUNGE_CHAT:ShowParsers()
	...
	_LOUNGE_CHAT_PARSERS = frame
		...
		header.Paint = function(me, w, h)
			...
			local close = vgui.Create("nebula.close", header)
			close:SetWide(th)
			close:Dock(RIGHT)
			close.DoClick = function(me)
				frame:Close()
		...
		local body = vgui.Create("nebula.scroll", frame)
		...
		body.Paint = function(me, w, h)
			...
			for i, ex in ipairs (self.MarkupsExamples) do
				...
				local pnl = vgui.Create("nebula.button", body)
				pnl:SetText("")
...
function LOUNGE_CHAT:ShowEmoticons()
	...
	_LOUNGE_CHAT_EMOTICONS = pnl
		...
		local scroll = vgui.Create("nebula.scroll", pnl)
		scroll:Dock(FILL)
...
function LOUNGE_CHAT:OpenChatbox(bteam)
	...
	_LOUNGE_CHAT.m_Entry.textentry:RequestFocus()
