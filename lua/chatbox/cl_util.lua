...
function LOUNGE_CHAT.Checkbox(text, cvar, parent)
	...
	pnl:SetDrawBackground(false)
	pnl:SetTall(draw.GetFontHeight("LOUNGE_CHAT_20_B") + 4)

		local lbl = LOUNGE_CHAT.Label(LOUNGE_CHAT.Lang(text), "LOUNGE_CHAT_20", LOUNGE_CHAT.Color("text"), pnl)
		lbl:Dock(FILL)

		local chk = vgui.Create("nebula.checkbox", pnl)
		chk:SetConVar(cvar)
		chk:SetWide(pnl:GetTall())
		chk:Dock(RIGHT)
		chk:DockMargin(2, 0, 2, 0)

	return pnl
end

function LOUNGE_CHAT.Button(text, parent, callback)
	local btn = vgui.Create("nebula.button", parent)
